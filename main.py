...
parser.add_argument(
    '--compress_rate',
    type=str,
    default='[0.0]+[0.6]*17+[0.5]*2+[0.6]*16+[0.5]*2+[0.6]*17',
    help='compress rate of each conv')
parser.add_argument(
    '--arch',
    type=str,
    default='resnet_56',
    choices=('resnet_50','vgg_16_bn','resnet_56','resnet_110','googlenet'),
...
parser.add_argument(
    ...
    default='A',
    choices=('A','B'),
    help='The chosen pruning method,A:bn priority pruning ; B:Full-layer redundant pruning')
...
def test(epoch, cov_id, optimizer, scheduler):
    ...
    if cov_id == len(convcfg):
        ...
        tb_writer.add_scalar("test_Acc", top1.avg, epoch)

    if epoch+1 == args.epochs :
        tb_writer.add_scalar("covid_Acc", best_acc, cov_id)
...
def main():
    for cov_id in range(args.start_cov, len(convcfg)):
        ...
        m.layer_mask(cov_id + 1, resume=args.resume_mask, param_per_cov=param_per_cov_dic[args.arch], arch=args.arch, way=args.pruning_way)
